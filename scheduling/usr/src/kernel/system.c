...
PRIVATE int do_setgroup(message *m_ptr) {
    register struct proc *proc_to_change;
    int group_a_count = 0;
    char new_group = toupper(m_ptr->m1_i1);
    int result = OK;

    proc_to_change = NIL_PROC;

    for(proc_to_change = BEG_USER_ADDR; proc_to_change < END_PROC_ADDR; ++proc_to_change) {
        if(proc_to_change->p_pid == m_ptr->m1_i2) {
            break;
        }

        if(proc_to_change->group == GROUP_A) {
            ++group_a_count;
        }
    }
    
    /* No process found */
    if (proc_to_change == NIL_PROC) {
        return ESRCH;
    }

    switch (new_group) {
        case 'A': proc_to_change->group = GROUP_A; break;
        case 'B': proc_to_change->group = GROUP_B; break;
        case 'C': proc_to_change->group = GROUP_C; break;
